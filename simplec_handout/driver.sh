...
run_test() {
	...
	cat temp.txt
	local pid=$!
	CATMATCH_PATTERN="$1" ./helper "$TEST_TEXT" >temp2.txt 2>/dev/null
	...
	diff -wB temp.txt temp2.txt &>/dev/null
	saved=$?
	if [ $saved -ne 0 ]; then
		sed -i '1d' temp.txt
		diff -wB temp.txt temp2.txt >/dev/null	# This is attempting to give points 
...
if [ $? -ne 0 ]; then
    echo "**THIS TEST TIMED OUT AFTER 5 SECONDS OR HAS A SEGFAULT**"
else
   	./helper "$TEST_TEXT" >temp2.txt 2>/dev/null
	diff -wB temp.txt temp2.txt &>/dev/null
	if [ $? -ne 0 ]; then
		sed -i '1d' temp.txt
		diff -wB temp.txt temp2.txt &>/dev/null	# This is attempting to give points 
